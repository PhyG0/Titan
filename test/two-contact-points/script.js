...
main.setTitle("Rectangle vs Rectangle Collision Detection");
...
let rectangle1 = new Rectangle(new Vector(-100, 0), 100, 100);

let rectangle2 = new Rectangle(new Vector(100, 0), 100, 100);
...
function update(dt) {
  ...
  if (isColliding) {
    ...
    for (let i = 0; i < collisionInfo.length; i++) {
      mf.points.push(collisionInfo[i].sp);
      mf.show(camera, ctx);
    }
  }
  if (isColliding) {
